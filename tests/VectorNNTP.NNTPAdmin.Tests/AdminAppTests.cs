...
public sealed class AdminAppTests : IClassFixture<SharedPgpKeyFixture>
{
    ...
    public async Task Inspect_DoesNotPost()
    {
        ...
        var fake = new FakeNntpAdminTransport
        {
    ...
    public async Task AuthFailure_DoesNotHeadOrPost()
    {
        var fake = new FakeNntpAdminTransport
        {
            AuthenticateException = new NntpAdminAuthenticationException("AUTHINFO PASS failed."),
        };
    ...
    private Task<int> RunAsync(string[] args, FakeNntpAdminTransport fake, StringWriter? stdout = null) =>
        ...
        bool includePgp = true)
    {
        ...
        {
            ["NntpAdmin:Host"] = "127.0.0.1",
            ["NntpAdmin:UseTls"] = "false",
            ["NntpAdmin:Port"] = "1199",
            ["NntpAdmin:From"] = "newsmaster@usenet.ninja",
            ["Nntpd:BindPort"] = "1199",
        ...
        if (includePgp)
        {
            values["NntpAdmin:Pgp:Enabled"] = "true";
            values["NntpAdmin:Pgp:PrivateKeyPath"] = _pgp.Path;
            values["NntpAdmin:Pgp:PrivateKeyPassphrase"] = TestPgpKeys.Passphrase;
        }
