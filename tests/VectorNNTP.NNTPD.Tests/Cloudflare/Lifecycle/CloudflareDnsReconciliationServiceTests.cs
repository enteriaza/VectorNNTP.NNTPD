...
public sealed class CloudflareDnsReconciliationServiceTests
{
    ...
    public async Task Host_RegistersDnsReconciliation_BeforeOtherServices()
    {
        ...
        Assert.Equal(typeof(NntpDbService), services[3].GetType());
        Assert.Equal(typeof(VectorNNTP.NNTPD.Newsgroups.NewsgroupCatalogueService), services[4].GetType());
        Assert.Equal(typeof(VectorNNTP.NNTPD.Moderation.ModeratorCatalogueService), services[5].GetType());
        Assert.Equal(typeof(VectorNNTP.NNTPD.History.HistoryWriteService), services[6].GetType());
        Assert.Equal(typeof(VectorNNTP.NNTPD.History.HistoryMaintenanceService), services[7].GetType());
        Assert.Equal(typeof(IncomingSpoolWriterService), services[8].GetType());
        Assert.Equal(typeof(VectorNNTP.NNTPD.Email.EmailDeliveryService), services[9].GetType());
        Assert.Equal(typeof(VectorNNTP.NNTPD.Transit.TransitDnsRefreshService), services[10].GetType());
        Assert.Equal(typeof(SessionStateService), services[11].GetType());
        Assert.Equal(typeof(VectorNNTP.NNTPD.Transit.TransitPeerStateService), services[12].GetType());
        Assert.Equal(typeof(NntpPlainListenerService), services[13].GetType());
        Assert.Equal(typeof(AcmeCertificateService), services[14].GetType());
        Assert.Equal(typeof(NntpTlsListenerService), services[15].GetType());
