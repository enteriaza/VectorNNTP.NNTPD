...
internal sealed class PgpVerifySigner : IDisposable
{
    ...
    public static bool TryCreate(PgpOptions options, out PgpVerifySigner signer, out string error)
    {
        ...
        {
            error = "NntpCancelMessage:Pgp:PrivateKeyPath is required when PGP is enabled.";
            return false;
        }
        ...
        if (!File.Exists(path))
        {
            error = "NntpCancelMessage:Pgp:PrivateKeyPath does not exist.";
            return false;
        }
        ...
        {
            error = "NntpCancelMessage:Pgp:PrivateKeyPath could not be read or parsed as an OpenPGP secret key.";
            return false;
        }
        catch (PgpException)
        {
            error = "NntpCancelMessage:Pgp:PrivateKeyPath could not be parsed as an OpenPGP secret key.";
            return false;
        }
        ...
        if (candidates.Count == 0)
        {
            error = bundle.Count == 0
                ? "NntpCancelMessage:Pgp:PrivateKeyPath could not be parsed as an OpenPGP secret key."
        ...
        if (wanted.Length == 0)
        {
            ...
            {
                error =
                    "NntpCancelMessage:Pgp:KeyId is required because the key file contains more than one signing-capable secret key.";
                return false;
            }
        ...
        else
        {
            ...
            if (selected is null)
            {
                error = "NntpCancelMessage:Pgp:KeyId did not match a signing-capable secret key in the file.";
                return false;
            }
        }
        ...
        catch (PgpException)
        {
            ...
            error = "NntpCancelMessage:Pgp:PrivateKeyPassphrase did not unlock the selected secret key.";
            return false;
        }
    ...
    private static bool TryValidateKeyFilePermissions(string path, out string error)
    {
        ...
        try
        {
            ...
            if ((mode & shared) != 0)
            {
                error = "NntpCancelMessage:Pgp:PrivateKeyPath must not be group- or world-accessible.";
                return false;
            }
        }
        catch (Exception ex) when (ex is PlatformNotSupportedException or IOException)
        {
            error = "NntpCancelMessage:Pgp:PrivateKeyPath permissions could not be verified.";
            return false;
        }
