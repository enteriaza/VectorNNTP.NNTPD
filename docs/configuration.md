...
`RabbitMqTopologyService` starts immediately after `RabbitMqService` and declares the required article-retrieval topology: twelve BackFiller `backfiller.<backbone>` fanout/quorum endpoints plus the internal `backfiller.storage` fanout/quorum endpoint. Exchange, queue, and routing-key names are trimmed and invariant-lowercased. BackFiller provider identifiers and `backfiller.storage` are application constants, not configuration keys. `backfiller.storage` is not a BackFiller provider. Declaration is fail-closed and uses RabbitMQ's idempotent declare/bind operations; incompatible existing topology is not deleted or rewritten. Connection-generation replacement does not redeclare topology.

`ArticleWorkRpcService` starts immediately after topology declaration. It attaches a shared NNTPD-owned RPC reply consumer and publishes BackFiller v1 article-work requests. Every request publication carries AMQP `RequestId`, a unique `CorrelationId`, `ReplyTo`, `ContentType=application/json`, and `Expiration=1000`. ARTICLE `<message-id>` uses that client; this phase classifies the RPC result and still maps it to the existing article-not-found NNTP reply because article retrieval is not implemented. The 500ms value is only the storage-to-provider fan-out grace (not a response-wait window). The 5s lookup deadline and 60s safety ceiling are application constants, not configuration keys.
