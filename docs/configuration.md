...
`RabbitMqTopologyService` starts immediately after `RabbitMqService` and declares the required article-retrieval topology: twelve BackFiller `grabbers.<backbone>` fanout/quorum endpoints plus the internal `storage.requests` fanout/quorum endpoint. BackFiller provider names and `storage.requests` are application constants, not configuration keys. `storage.requests` is not a BackFiller provider. Declaration is fail-closed and uses RabbitMQ's idempotent declare/bind operations; incompatible existing topology is not deleted or rewritten. This phase still does not publish or consume messages and does not define an RPC or storage-response contract. Connection-generation replacement does not redeclare topology.
