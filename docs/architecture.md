...
AUTHINFO PASS password
    ...
    → SessionState admission (`account_session_limit` is the cluster-wide
      ...
      R-account rate requires every new session to take the distributed path so
...
`VectorNNTP.NNTPD.SessionState.BytesAccounting` owns cluster-wide remaining-byte quota for AUTHINFO `account_type` **B** (byte-oriented) accounts. It is a bounded context inside SessionState, not a second application service. TransitPeerState remains a separate peer-connection context. `account_byte_limit` is **remaining** bytes: `> 0` means bytes remain, `0` means exhausted, and a negative value is never allowed. There is no `0 = unlimited` sentinel for B accounts; an effectively unlimited B account is provisioned with a large positive remaining quota (for example 10 TB). R accounts, unauthenticated sessions, transit peers, and newsmaster/admin identities whose `AccountPolicy` is not a B policy do not participate. Account type comes from the authenticated `NntpAccountPolicy`, never from the username.
...
`VectorNNTP.NNTPD.SessionState.RateLimiting` owns cluster-wide outbound download-rate allocation for AUTHINFO `account_type` **R** (rate-oriented) accounts. It is a bounded context inside SessionState, not a second application service. `account_rate_limit` is the account-wide aggregate cap in **bits per second (bps)**. Examples: `240` = 240 bps = 30 B/s; `1,000,000` = 1 Mbps = 125,000 B/s; `10,000,000` = 10 Mbps = 1,250,000 B/s. `0` is unlimited (existing policy) and does not participate in allocation. This is not the B-account `0 = exhausted` rule. The denominator is the live cluster-wide authenticated session count from the SessionState session HASH — never `account_session_limit`. Conversion is integer `floor(bps / 8)` to account bytes/sec, then `floor(accountBytes / activeSessions)` so `sum(caps) <= account bytes/sec`. A positive configured rate below 8 bps floors to 0 bytes/sec and is blocked (`cap = -1`), never treated as unlimited. B accounts never enter this allocator; R accounts never consume byte quota.
