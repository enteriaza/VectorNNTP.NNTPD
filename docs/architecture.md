...
`VectorNNTP.NNTPD.SessionState.RateLimiting` owns cluster-wide outbound download-rate allocation for AUTHINFO `account_type` **R** (rate-oriented) accounts. It is a bounded context inside SessionState, not a second application service. `account_rate_limit` is the account-wide aggregate cap in decimal SI megabits per second. `0` is unlimited (existing policy) and does not participate in allocation. This is not the B-account `0 = exhausted` rule. The denominator is the live cluster-wide authenticated session count from the SessionState session HASH — never `account_session_limit`. The converged per-session cap is `floor((rateMbps × 125_000) / activeSessions)` bytes/sec so `sum(caps) <= account bytes/sec`. B accounts never enter this allocator; R accounts never consume byte quota.
