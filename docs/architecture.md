...
│  - SessionStateService (leases + source IPs + byte quota)   │
...
AUTHINFO PASS password
    ...
    → SessionState admission (`account_session_limit` is the cluster-wide
      ...
      only while `account_session_limit` is 0 and this node holds a live source
      lease; a finite session limit requires every new session to take the
      distributed path so the global session count stays exact. Leases last 30s
