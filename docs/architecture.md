...
│  - ArticleWorkRpcService (RPC reply consumer + orchestration)│
...
Shared ACME, Cloudflare DNS, and bind-address types live in `VectorNNTP.Common` (historical namespaces `VectorNNTP.NNTPD.Acme`, `VectorNNTP.NNTPD.Cloudflare`, `VectorNNTP.NNTPD.Networking`, `VectorNNTP.NNTPD.Configuration`). NNTPD and BackFiller both consume that library. There is one Common-owned configuration contract (`AcmeCloudflareOptions` at the configuration root) and one shared environment-variable contract: uppercase `VECTOR__` names with no application-specific identifier (`VECTOR__CLOUDFLAREAPIKEY`, `VECTOR__ACMECERTIFICATEPASSWORD`, `VECTOR__CLOUDFLAREZONEID`, `VECTOR__BINDADDRESS`, `VECTOR__BINDPORT`, `VECTOR__BINDPORTTLS`). Application identity and NNTPD-only secrets stay on `NNTPD__*` / `BACKFILLER__*`. Values are case-sensitive. BindAddress/BindPort resolution and Cloudflare A/AAAA follow the same listener-address rules. ACME account, DNS-01, issuance, and renewal are shared; BackFiller requests only `{BackFillerFqdn}` and does not request `news.usenet.ninja`. BackFiller is TLS-only: the listener binds `BindPortTls` only, and startup fails unless a usable ACME certificate is ready before the listener starts. Application lifecycle wrappers stay in each host. API client and reconciler types live under Common `Cloudflare/`; bind expansion under Common `Networking/`. Credentials and raw Cloudflare response bodies are never logged in exception messages.
...
`RabbitMqTopologyService` is the next layer. It owns declaration of the required article-retrieval topology and does not own connection lifecycle, reconnect, credentials, publishers, or consumers. Startup order is `RedisService` → `RabbitMqService` → `RabbitMqTopologyService` → `ArticleWorkRpcService` → `NntpDbService`. Topology declaration is fail-closed: missing current connection, rejected exchange/queue/bind, or incompatible existing entities (including a classic queue where quorum is required) prevent `Running`. Declarations use RabbitMQ's normal idempotent declare/bind operations. NNTPD never deletes, purges, or mutates existing entities to "fix" them.

`ArticleWorkRpcService` owns NNTPD article-work RPC. It opens caller-owned publish and consume channels on the current `RabbitMqService` generation, declares one exclusive auto-delete reply queue, and consumes responses onto a shared correlation router. Each logical lookup has one UUID `RequestId` placed on both the AMQP `RequestId` property and JSON `requestId`. Every request publication uses a fresh AMQP `CorrelationId`, `ReplyTo` of the shared NNTPD reply queue, `ContentType=application/json`, and `Expiration=1000`. Worker responses use the same 1-second message TTL. RabbitMQ expires undelivered or leftover messages; NNTPD does not delete, purge, or send cancellation messages to clean those queues. Request TTL is not an end-to-end retrieval limit: a worker may consume a request before 1 second and reply later, and NNTPD still accepts that reply during the active 5-second lookup. NNTPD in-process correlation registrations are removed as soon as the logical lookup completes (first valid Success, 5-second deadline, 60-second safety ceiling, caller cancellation, or shutdown). Late replies for unknown correlations are ignored and cannot resurrect or mutate a completed lookup. Outstanding provider publications are observed but not awaited by the reader hot path. ARTICLE `<message-id>` publishes to `backfiller.storage` at T+0 and starts a 500ms storage-to-provider grace concurrently. The 500ms value is not a response-wait or qualification window: usable responses are processed immediately. Only `Success` is aggregate-terminal (first valid Success wins and completes the lookup at once). `ArticleNotFound`, `InvalidArticle`, and `InvalidRequest` are source-local and do not complete the multi-source lookup or suppress other destinations. If the lookup is still unresolved at T+500ms, NNTPD fans out concurrently to the twelve `backfiller.{provider}` exchanges. The operational not-found deadline is 5 seconds from request start; 60 seconds is an absolute safety ceiling. This phase classifies the RPC result only and does not retrieve article bytes or emit ARTICLE success responses. Connection-generation replacement rebuilds the RPC session; a stale generation cannot publish into or mutate the replacement session.
...
Each endpoint is a durable fanout exchange (not auto-delete), a durable non-exclusive non-auto-delete quorum queue (`x-queue-type=quorum`), and a binding that uses the same name as the routing key. NNTPD does not consume these request queues. Article-work RPC is implemented: NNTPD publishes BackFiller v1 JSON requests and consumes replies on its exclusive reply queue. Broker-side message expiry uses `Expiration=1000` on request and response messages. NNTPD correlation state is separate and is removed when the logical lookup completes.
...
The initial connect is fail-closed: invalid configuration or a failed startup connect prevents `Running`. After start, RabbitMQ is a runtime invariant, not an abandonable dependency. Client automatic recovery is disabled. Loss of the current generation starts one reconnect loop that continues with the configured backoff until a new generation is installed or shutdown cancels the loop. There is no give-up threshold. Callers use `TryGetCurrent` for a non-owning generation snapshot (`RabbitMqConnectionHandle`). The handle does not pin the connection: `IsCurrent` is a point-in-time observation under the service gate, not a lifetime lease. After loss the handle becomes not-current (closed and/or unpublished) while recovery may dispose generation `N` concurrently with any caller that still holds that instance. Checking `IsCurrent` immediately before an operation does not keep the connection alive for that operation. Future RPC must detect staleness via `Generation` / `IsCurrent` and will need a separate channel lease (or equivalent pin) if in-flight work cannot tolerate concurrent dispose. Callers must not dispose the connection. Only the current generation may publish readiness, request recovery, or be retired as current. Shutdown cancels the watch task, waits for it, and disposes the current connection once. Shutdown is idempotent and coordinated by `ApplicationServiceManager` like other application services.
